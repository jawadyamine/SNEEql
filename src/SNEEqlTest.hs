...
import System.IO
import qualified Control.Exception hiding (evaluate)
...
{-- Demo and test stuff --}
jawad= "select RSTREAM Rons.temp, Als.temp FROM Rons[NOW], Als[NOW] where Rons.light < 100 and Als.light < 100;"
jawad2="SELECT RSTREAM Avg(Rooms.temp) FROM Rooms[NOW] where temp > 30 and light < 100;"
jawad3="SELECT RSTREAM Inflow.pressure, Inflow.id FROM InFlow[NOW];"
...
demo query = do
	...
	sysCat <- readSysCat "building-schemas.xml"
...
--	print environment
	...
	print operator
	Control.Exception.bracket (openFile "building-query1.txt" WriteMode) hClose
	        (\h -> hPutStrLn h (show operator))
--	print "cost"
--	let timeCost = evaluateDemo operator
--	print timeCost
	print "Evaluate"
	results <- evaluateAndDisplaySome environment operator 20
	print results
...
run query = do
	...
	-- print "SysCat"
	-- sysCat <- readSysCat "declarations.txt"
	sysCat <- readSysCat "ssdbm-10-node-schemas.xml"
	-- print sysCat
	print "Query:"
	print query
	-- print "Tokens:"
...
translate query declFile outfile = do
	...
	let (environment, queryType) = typeFullQuery sysCat ast
	let operator = translatePlusFullQuery environment ast
	Control.Exception.bracket (openFile outfile WriteMode) hClose
	        (\h -> hPutStrLn h (show operator))
	print "success"
